...
describe('src/index.ts', function () {
    ...
    describe('#httpProxy.createProxyServer() method with error response', function () {
    ...
    describe('#createProxyServer with xfwd option', function () {
        it('should not throw on empty http host header', function (done) {
            ...
            var socket = connect({ port: ports.proxy }, function () {
            ...
            //      http.http.request('http://127.0.0.1:' + ports.proxy, function() {}).end();
        })
    });
    ...
    describe('#createProxyServer using the ws-incoming passes', function () {
        it('should proxy the websockets stream', function (done) {
            var ports = { source: gen.port, proxy: gen.port },
                ...
                destiny = new WebSocketServer({ server });
            ...
            server.on('listening', startWebSockets);
        ...
        it('should proxy a socket.io stream', function (done) {
            ...
            function startSocketIo() {
                var client = clientIO.connect('ws://127.0.0.1:' + ports.proxy);
        ...
        it('should emit open and close events when socket.io client connects and disconnects', function (done) {
            ...
            function startSocketIo() {
                var client = clientIO.connect('ws://127.0.0.1:' + ports.proxy, { rejectUnauthorized: null });
                client.on('connect', function () {
                    client.disconnect();
                });
            }
